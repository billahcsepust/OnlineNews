...
@section('content')
    <link rel="stylesheet" href="{{asset('public/admin/assets/css/lib/datatable/dataTables.bootstrap.min.css')}}">
        ...
        <div class="content mt-3">
            ...
            <a href="{{url('/back/categories/create')}}" class="btn btn-primary pull-right">Create</a>
            @endpermission
                        </div>
                        <div class="card-body">
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $row->name }}</td>
                        <td> 
                {{ Form::open(['method'=>'PUT','url'=>['/back/categories/status/'.$row->id],'style'=>'display:inline']) }}
                @if($row->status===1)
            ...
            @permission(['Post Add','All','Post Update'])
                <a href="{{ url('/back/categories/edit/'.$row->id )}} " class="btn btn-primary">Edit</a>
            @endpermission

            @permission(['Post Add','All'])
                {{ Form::open(['method'=>'DELETE','url'=>['/back/categories/delete/'.$row->id],'style'=>'display:inline']) }}
